...
- `Alt+Enter` - открыть файл в редакторе
...
- **Read-only просмотр** файлов с заголовком и статусной строкой
- **Навигация**: построчная прокрутка (↑↓/jk), PageUp/PageDown, `g`/`G` для перехода к началу/концу
- **Повторная загрузка** (`Ctrl+R`) с индикаторами статуса
- **Показ метаданных**: размер, количество строк, время последней модификации
- **Расширяемый каркас** для будущих функций редактирования, подсветки и поиска
