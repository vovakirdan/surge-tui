...
- `Ctrl+,` - настройки
- `Ctrl+Q` - выход
- `Ctrl+1/2/3/4` - быстрый переход к Проекту / Файлам / Редактору / Сборке
...
- `S` - фильтр по `.sg`
- `n` - создать новый файл/папку
- `r` - переименовать
- `Del` - удалить
- `Ctrl+R` - обновить дерево файлов
...
### 🔄 Этап 2: Минимальный MVP (В ПРОЦЕССЕ)
- [x] 🎉 **Экран "Проект"** - дерево файлов с навигацией, статус проекта
- [x] 🎉 **Экран "Настройки"** - полнофункциональные настройки с live preview
- [x] 🎉 **Файловый менеджер** - навигация, создание/удаление/переименование, фильтры
...
- **Асинхронная загрузка** с индикаторами состояния
- **Обработка ошибок** с возможностью повтора (r)
- **Responsive дизайн** под размер терминала

### 🆕 Реализованные возможности файлового менеджера:
- **Навигация** по дереву проекта, раскрытие директорий
- **Создание** новых файлов (`n`) и папок (`Shift+N`) в выбранной директории
- **Переименование** выделенного элемента (`r`)
- **Удаление** с диалогом подтверждения (`Delete`)
- **Фильтры** скрытых файлов (`h`) и `.sg`-расширения (`s`)
- **Информационная панель** с подробностями по выбранному элементу
- **Статусные сообщения** об ошибках операций
